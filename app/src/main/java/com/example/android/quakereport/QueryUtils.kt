...
internal constructor() {
    ...
    fun extractEarthquakes(): ArrayList<Earthquake> {
        ...
        try {
            ...
            for (i in 0..9) {
                ...
                println(properties.getString("location"))

                //Split the string that comes from mock and pass it to the constructor
                if (properties.getString("location").contains("of")) {

                    val parts = properties.getString("location").split(" of ".toRegex()).dropLastWhile {
                        it.isEmpty()
                    }.toTypedArray()
                    ...
                    earthquakes.add(Earthquake(properties.getDouble("mag"), parts[0], parts[1],
                            convertToDateOrTime(properties.getLong("time"), "MMM DD, yyyy"),
                            convertToDateOrTime(properties.getLong("time"), "HH:mm")))

                } else {
                    earthquakes.add(Earthquake(properties.getDouble("mag"),
                            properties.getString("location"), convertToDateOrTime(properties.getLong("time"), "MMM DD, yyyy"),
                            convertToDateOrTime(properties.getLong("time"), "HH:mm")))
                }
            }
    ...
    private fun convertToDateOrTime(timeInMilliseconds: Long, outputMask: String): String {
